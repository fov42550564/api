...
// 常亮设置
export const RRT_FLAW_HOUSE = '人工处理仓库'; // 人工处理仓库
...
0->3->4->9 |
===========
同城配送状态图: (当同城配送的在扫描装车后，那么它和它之前的货单就在待交货状态)
(0->)1->2->5    |
                |->6->14->15
(0->)1->3->4->5 |
===========
上门自提状态图:(当上门自提的在等待上门自提，那么它之前的货单就在待交货状态， 上门自提的在扫描装车后，那么它和它之前的货单就在待交货状态)
         |->13->15
1->5->12 |
         |->6->14->15
*/
...
schema.virtual('state').get(function () {
    return this.stateList && this.stateList[0].state;
});
...
* PDShopServer/project/App/models/UnionPayModel.js
...
const schema = new Schema({
    ...
    txnAmt: { type: Number }, // 总金额

    // 回包信息
    tn: { type: String }, // 预支付交易会话 Id
    accessType: { type: String }, // 接入类型
    merId: { type: String }, // 商户 Id
    txnType: { type: String }, // 交易类型
    bizType: { type: String }, // 产品类型
    version: { type: String }, // 版本号

    // 支付信息
    queryId: { type: String }, // 查询流水号
    traceNo: { type: String }, // 系统跟踪号
    traceTime: { type: String }, // 交易传输时间
    settleDate: { type: String }, // 清算日期
    settleCurrencyCode: { type: String }, // 清算币种
    settleAmt: { type: String }, // 清算金额

    // 状态
    state: { type: Number, default: 0 }, // 未支付
    payedTime: { type: Date }, // 支付完成时间，使用通知接口的 traceTime 赋值
    createTime: { type: Date, default: Date.now }, //创建时间
});
...
export default mongoose.model('UnionPay', schema);
...
const schema = new Schema({
    ...
    // 基本信息
    warehousePartmentId: { type: Schema.Types.ObjectId, ref: 'ShopPartment' }, // 对应的库管部
...
virtualObjectById(schema, 'warehousePartmentId');

const transform = {
    virtuals: true,
    transform: function (doc, ret, options) {
        formatTime(ret, 'createTime', 'modifyTime');
        formatObjectById(ret, 'warehousePartmentId');
        return ret;
    },
};
schema.options.toJSON = transform;
...
const schema = new Schema({
    ...
    // 状态
    state: { type: Number, default: 0 }, // 未支付
    payedTime: { type: Date }, // 支付完成时间，使用通知接口的 time_end 赋值
    createTime: { type: Date, default: Date.now }, //创建时间
});
...
export UnionPayModel from './UnionPayModel';
...
function registerSocketRouter (server, root, sockets) {
    ...
    io.on('connection', (socket) => {
        ...
        console.log('connection by', socket.userId, '; online: ', _.keys(io.sockets.sockets).length);
        socket.on('disconnect', () => {
            console.log('socket disconnect by', socket.userId, '; online: ', _.keys(io.sockets.sockets).length);
        });
        ...
        if (typeof func === 'object') {
            ...
            router.post(url, async (req, res) => {
                if (process.env.NODE_ENV === 'production') {
                    ...
                    let result = await func(req.body, { req, io });
                    ...
                    console.log(url + ' recv:', req.body, req.file || '');
                    let result = await DEBUG(func, req.body, { req, io });
                    _.includes(LOG_FILTERS, api) || console.log(url + ' send:', JSON.stringify(result, null, 2));
                    setTimeout(() => {
                        if (!result || typeof result === 'number') {
                            res.sendStatus(result || 500);
        ...
        if (typeof func === 'object') {
            ...
            router.get(url, async (req, res) => {
                if (process.env.NODE_ENV === 'production') {
                    ...
                    let result = await func(req.query, { req, io });
                    ...
                    console.log(url + ' recv:', req.query);
                    let result = await DEBUG(func, req.query, { req, io });
                    _.includes(LOG_FILTERS, api) || console.log(url + ' send:', result.readable ? 'image' : result);
                    setTimeout(() => {
                        if (!result || typeof result === 'number') {
                            res.sendStatus(result || 500);
...
export function getAgentFee (fee, rate, weight = 1) { // 只有计算运费的时候才需要传入weight, 计算价格的时候不需要
    let agentFee;
    if (rate.profitCount) {
        agentFee = fee + rate.profitCount * weight;
    } else {
        agentFee = (1 + (rate.profitRate || 0)) * fee;
    }
    return _N(agentFee);
}
...
* PDShopServer/project/App/routers/sockets/index.js
...
* PDShopServer/project/App/routers/sockets/shop/index.js
...
// unionPay
export unionPayGetUnifiedOrder from './unionPay/getUnifiedOrder'; // 统一下单(App支付)
export unionPayGetUnifiedOrderForPC from './unionPay/getUnifiedOrderForPC'; // 统一下单(扫码支付)
...
export getRoadmapListWithOrder from './roadmap/getRoadmapListWithOrder'; // 获取货单的价格列表
export getRoadmapListWithTruck from './roadmap/getRoadmapListWithTruck'; // 获取货车的价格列表
...
// weixinPay
export weixinPayNotifyCallBack from '../libs/weixinPay/notifyCallBack'; // 微信支付通知回调
// unionPay
export unionPayNotifyCallBack from '../libs/unionPay/notifyCallBack'; // 银联支付通知回调
...
export getLastOrder from './receivePartment/getLastOrder'; // 收货部获取前一个单的信息

// warehousePartment
export scanOrderByWareHousePartment from './warehousePartment/scanOrder'; // 库管部扫描货物
...
export letTruckPass from './securityCheckPartment/letTruckPass'; // 保安部放行
...
* PDShopServer/project/App/routers/sockets/shop/task/TEST_RQ.js
...
export default async ({
    ...
    // io.emitTo(username, 'TEST_RS', { username, password });
};
...
export default async ({
    ...
    const clientAccount = await AccountModel.findOne({ where: { targetId: _T(client.id), type: CONSTANTS.AT_CLIENT } });
...
export default async ({
    ...
    } else if (context.state !== CONSTANTS.OS_READY_PRINT_BAR_CODE) {
    ...
    if (!fromPC) {
        io.emitTo(userId, 'RP_REFRESH_ORDER_NF'); // 发送刷新货单的通知
    }
...
export default async ({
    ...
    const order = await OrderModel.findByIdAndUpdate(preOrderId, {
        ...
        stateList: [ { state: CONSTANTS.OS_READY_PRINT_BAR_CODE, count: totalNumbers } ],
        placeOrderTime: Date.now(),
        $unset: { groupId: 1, shopId: 1, startPointLastCode: 1, startPoint: 1 },
    });
    const error = await setAgentOrderFee(member.agent.referShopId, order);
    if (error) {
        order.deductError = true;
...
import { _T, _F, _Y, checkVerifyCode, actionLog } from '../../../../utils';
import CONSTANTS from '../../../../constants';
...
export default async ({
    ...
    verifyCode, //如果是物流公司代签，需要填写收货人的验证码
}) => {
    const client = await ClientModel.findById(userId);
    if (!client) {
        return { msg: '没有该用户' };
    ...
    if (currentOrder.totalDesignatedFee + currentOrder.proxyCharge + currentOrder.additionalFee > 0) { // 需要支付的情况
        if (userId === currentOrder.receiverId) {
            return { msg: '该货单需要支付' };
        }
    } else if (_T(client.shipperId) === _T(currentOrder.shipperId)) {
        const success = await checkVerifyCode(currentOrder.receiverPhone, verifyCode);
        if (!success) {
            return { msg: '无效验证码' };
        }
    }
...
async function getPageData (receiverId, type, params) {
    ...
    let criteria = {};
    if (type === 'unsend') {
        criteria = { 'stateList.state': { $gte: CONSTANTS.OS_READY_PRINT_ORDER, $lte: CONSTANTS.OS_READY_START_OFF } };
    } else if (type === 'onway') {
        criteria = { 'stateList.state': { $gte: CONSTANTS.OS_ON_THE_WAY, $lte: CONSTANTS.OS_ON_TRANSFER } };
    ...
    criteria = getKeywordCriteriaForOrder(keyword, omitNil({ ...criteria, receiverId, isTransferOrder: false }));
...
// ['待发货', '运输中', '待收货', '已完成']
const TYPES = ['unsend', 'onway', 'toreceive', 'success'];
export default async ({ userId, shopId, type, ...params }) => {
    if (_.includes(TYPES, type)) {
        ...
        return {
            success: true,
            context: {
                unsend: await getPageData(userId, 'unsend', params),
                onway: await getPageData(userId, 'onway', params),
...
async function getPageData (senderId, type, params) {
    ...
    if (type === 'topay') {
        ...
        criteria = { 'stateList.state': { $gte: CONSTANTS.OS_READY_PRINT_ORDER, $lte: CONSTANTS.OS_READY_RECEIVE } };
    } else if (type === 'success') {
        criteria = { 'stateList.state': CONSTANTS.OS_RECEIVE_SUCCESS };
    }
    if (fromPC) {
...
// ['待支付', '运输中', '已完成']
const TYPES = ['topay', 'onway', 'success'];
export default async ({ userId, type, ...params }) => {
    const client = await ClientModel.findById(userId)
    .populate({
        path: 'shipperId',
...
export default async ({
    ...
    for (const orderId of orderIdList) {
        ...
        order.stateList = [ { state: CONSTANTS.OS_READY_PRINT_ORDER, count: order.totalNumbers } ];
        await order.save();
        receiveMemberIdList = _.union(receiveMemberIdList, [order.receiveAgentMemberId]);
    }

    actionLogWithList(userId, 'Client', orderIdList.map(o => ({ id: o, model: 'Order' })), 'payForAgentOrder');

    receiveMemberIdList.forEach(receiveMemberId => { // 发送刷新货单的通知
        io.emitTo(receiveMemberId, 'RP_REFRESH_ORDER_NF');
    });
...
export default async ({
    ...
    for (const orderId of orderIdList) {
        ...
        order.stateList = [ { state: CONSTANTS.OS_READY_PRINT_ORDER, count: order.totalNumbers } ];
        await order.save();
        receiveMemberIdList = _.union(receiveMemberIdList, [order.receiveMemberId]);
    }
...
```js
import { ClientModel, OrderModel, OrderGroupModel } from '../../../../models';
import { actionLog } from '../../../../utils';
...
export default async ({
    ...
    if (member.shipper) {
        senderId = member.shipper.chairManId;
    } else if (member.agent) {
        senderId = member.agent.chairManId;
    }
...
export default async ({ userId, fromPC, pageNo, pageSize }) => {
    ...
    const senderId = member.shipper ? member.shipper.chairManId : member.agent ? member.agent.chairManId : userId; // 物流公司的人员预下单以物流公司的董事长为发货人
...
// 如果是普通用户，获取的是其下的预下单
// 如果是物流公司，获取的是其下的预下单和运输中的货单(只算长途运输)
// 如果是收货点，获取的是待发货的货单(收货点没有预下单的功能)
export default async ({ userId, fromPC, pageNo, pageSize }) => {
    const member = await ClientModel.findById(userId)
    .populate({
        path: 'shipperId',
    ...
    let criteria;
    if (member.agentId) { // 收货点
        criteria = { 'stateList.state': CONSTANTS.OS_READY_SEND, agentId: member.agentId, groupId: { $exists: false } };
    } else if (member.shipper) { // 物流公司
        criteria = { $or: [
            { senderId: member.shipper.chairManId, 'stateList.state': CONSTANTS.OS_PREORDER, groupId: { $exists: false } },
            { shipperId: member.shipper.id, 'stateList.state': CONSTANTS.OS_ON_THE_WAY, isCityDistribute: false, groupId: { $exists: false } },
        ] };
    } else {
        criteria = { senderId: userId, 'stateList.state': CONSTANTS.OS_PREORDER, groupId: { $exists: false } };
    }
...
```js
import mongoose from 'mongoose';
import { RoadmapModel, ShopModel, AgentModel } from '../../../../models';
import { getRegionProfitRateByEndPoint } from '../../libs/roadmap';
import { getAgentProfitRate } from '../../libs/order';
import { getAgentFee } from '../../../../utils';
import CONSTANTS from '../../../../constants';
import _ from 'lodash';
...
export default async ({
    ...
    if (agentId) {
        ...
        if (roadmap) {
            const agentRate = await getAgentProfitRate(agent._id, agent.referShopId, isCityDistribute, endPointLastCode);
            roadmap.price = getAgentFee(roadmap.price, agentRate);
            roadmap.minFee = getAgentFee(roadmap.minFee, agentRate);
...
* PDShopServer/project/App/routers/posts/client/roadmap/getRoadmapListWithOrder.js

```js
import mongoose from 'mongoose';
import { OrderModel, ShopModel } from '../../../../models';
...
export default async ({
    userId,
    orderId, // 货单号
    startPointLastCode, // 始发地的lastCode
    shopId, // 指定的分店
    location, // 我的位置
    orderBy, // 排序方式，0：价格排序，1：时间排序，2：亲签率排序，3：距离排序
    fromPC,
    ...
    const roadmapList = await searchRoadmapListWithShopList({
        shopList,
        fromPC,
        pageNo,
        pageSize,
    ...
    return { success: true, context: roadmapList };
};
...
```js
import mongoose from 'mongoose';
import { OrderModel, ShopModel, AgentModel } from '../../../../models';
import { searchRoadmapListWithShop, searchRoadmapListWithShopList } from '../../libs/roadmap';
import { getAgentProfitRate } from '../../libs/order';
import { getAgentFee } from '../../../../utils';
import { settingMgr } from '../../../../manager';
...
export default async ({
    ...
    if (agentId) {
        ...
        if (roadmap) {
            ...
            roadmap.agentName = agent.name;
            roadmap.agentAddress = agent.address;
            roadmap.transportFee = getAgentFee(roadmap.transportFee, agentRate, settingMgr.getFloatWeight(weight, size));
            roadmap.price = getAgentFee(roadmap.price, agentRate);
            roadmap.minFee = getAgentFee(roadmap.minFee, agentRate);
        }
...
```js
import mongoose from 'mongoose';
import { ClientModel, OrderGroupModel, ShopModel, AgentModel } from '../../../../models';
    ...
    if (agentId) {
        ...
        for (const order of orderIdList) {
            ...
            if (!roadmapList.length) {
                ...
                const agentRate = await getAgentProfitRate(agentId, agent.referShopId, order.isCityDistribute, order.endPointLastCode);
                const agentFee = getAgentFee(roadmapList[0].transportFee, agentRate, settingMgr.getFloatWeight(order.weight, order.size));
                totalTransportFee += agentFee;
            }
        }
        ...
        for (const agent of agents) {
            ...
            for (const order of orderIdList) {
                ...
                if (!roadmapList.length) {
                    ...
                    const agentRate = await getAgentProfitRate(agent.id, agent.referShopId, order.isCityDistribute, order.endPointLastCode);
                    const agentFee = getAgentFee(roadmapList[0].transportFee, agentRate, settingMgr.getFloatWeight(order.weight, order.size));
                    totalTransportFee += agentFee;
                }
            }
...
* PDShopServer/project/App/routers/posts/client/roadmap/getRoadmapListWithTruck.js

```js
import mongoose from 'mongoose';
import { ClientModel, TruckModel, ShopModel } from '../../../../models';
import { searchRoadmapListWithShop } from '../../libs/roadmap';
import CONSTANTS from '../../../../constants';
...
* PDShopServer/project/App/routers/posts/client/unionPay/getUnifiedOrder.js

```js
import { ClientModel, UnionPayModel } from '../../../../models';
import { _F, _T, hasAuthority, actionLog } from '../../../../utils/';
import CONSTANTS from '../../../../constants';
import api from '../../libs/unionPay/api';
import config from '../../../../../config';
import { rechargeForClient } from '../../libs/account';
...
export default async ({
    ...
    amount = amount / config.appPayRate;
    if (amount < 1) {
        return { msg: '充值金额必须大于' + config.appPayRate / 100 };
    }
    return new Promise(async resolve => {
        const client = await ClientModel.findById(userId);
        if (!client) {
        ...
        const params = {
            orderDesc: body,
            orderId: _T(order.orderId),
            txnAmt: amount,
        };
        const result = await api.unifiedorder(params);
        if (result.success) {
            order.tn = result.context.tn;
            order.accessType = result.context.accessType;
            order.merId = result.context.merId;
            order.txnType = result.context.txnType;
            order.bizType = result.context.bizType;
            order.version = result.context.version;
            await order.save();
        }
        actionLog(userId, 'ShopMember', amount, 'Number', 'unionPayGetUnifiedOrder');
...
* PDShopServer/project/App/routers/posts/client/unionPay/getUnifiedOrderForPC.js
...
export default async ({
    ...
    return new Promise(async resolve => {
        ...
        actionLog(userId, 'ShopMember', amount, 'Number', 'unionPayGetUnifiedOrderForPC');
...
export default async ({
    ...
    return new Promise(async resolve => {
        ...
        actionLog(userId, 'Client', amount, 'Number', 'weixinPayGetUnifiedOrder');
...
export default async ({
    ...
    return new Promise(async resolve => {
        ...
        actionLog(userId, 'Client', amount, 'Number', 'weixinPayGetUnifiedOrderForPC');
...
```js
import mongoose from 'mongoose';
import { OrderModel, RoadmapModel } from '../../../../models';
import { settingMgr } from '../../../../manager';
import getRegionProfitRateByEndPoint from '../roadmap/getRegionProfitRateByEndPoint';
import getAgentProfitRate from './getAgentProfitRate';
import CONSTANTS from '../../../../constants';
import { getAgentFee } from '../../../../utils';
import moment from 'moment';
...
export default async (
    ...
    const agentRate = await getAgentProfitRate(agentId, shopId, isCityDistribute, endPointLastCode);
    const totalFee = getAgentFee(fee, agentRate, weight);
    return {
        fee, // 分店显示的价格
...
* PDShopServer/project/App/routers/posts/libs/order/getAgentProfitRate.js
...
export default async (agentId, shopId, isCityDistribute, endPointLastCode) => {
    let addressList = [0];
    while (true) {
...
export getAgentProfitRate from './getAgentProfitRate'; // 获取收货点的方向提成
...
export default async (order, profitRate) => {
    ...
    if (item) {
        ...
        if (err && err !== ERROR_HAVE_PAYED) {
            order.warehouse = CONSTANTS.RRT_FLAW_HOUSE;
            order.deductError = true;
        } else {
...
* PDShopServer/project/App/routers/posts/libs/unionPay/api.js
...
const CERTS_PATH = path.resolve(process.cwd(), '.certificates', 'unionPay');
const BASE_SERVER = '.test.95516.com';// 测试环境
// const BASE_SERVER = '.95516.com'; //正式环境
const SERVER = 'https://gateway' + BASE_SERVER + '/gateway/api/';
...
const CONFIG = {
    ...
    accessType: '0', // 0：商户直连接入 1：收单机构接入
    channelType: '08', // 渠道类型
    backUrl: 'http://120.78.65.221/api/unionPayNotifyCallBack', // 后台通知地址
    currencyCode: '156', // 交易币种，币种格式必须为3位代码，境内客户取值：156（人民币） 固定填写：156
    merId: '777290058153875', //商户代码
};

const STORE = {};

// const frontTransUrl = SERVER+'frontTransReq.do';//前台交易请求地址
// const backTransUrl = SERVER+'backTransReq.do';//后台交易请求地址
// const singleQueryUrl = SERVER+'queryTrans.do';//单笔查询请求地址
// const batchTransUrl = SERVER+'batchTrans.do';//批量查询请求地址
// const fileTransUrl = 'https://filedownload'+BASE_SERVER+'/';//文件传输类交易地址
// const appTransUrl = SERVER+'appTransReq.do';//APP交易请求地址
// const cardTransUrl = SERVER+'cardTransReq.do';

const initSignCert = (path, password) => {
    return new Promise(resolve => {
        pem.readPkcs12(path, { p12Password: password }, (err, pfx) => {
            if (err) {
                resolve();
                return;
            }
            ...
            STORE.cert = {
                certId: new jsrsasign.BigInteger(c.getSerialNumberHex(), 16).toString(),
                privateKey: jsrsasign.KEYUTIL.getKey(pfx.key),
            };
            resolve();
        });
    });
...
const post = (url, data) => {
    return new Promise(resolve => {
        superagent.post(url).set({ 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }).send(data).end((error, res) => {
            if (!error && res.statusCode === 200) {
                const ret = querystring.parse(res.text);
                if (ret.respCode === '00') {
                    resolve({ success: true, context: _.omit(ret, 'signature', 'signPubKeyCert') });
                } else {
                    resolve({ msg: ret.respMsg });
                }
            } else {
                resolve({ msg: res.text });
            }
        });
    });
};
const createLinkString = (param, sort, encode) => {
    if (sort) {
        const obj = {};
        const keys = _.keys(param).sort();
    ...
    if (encode) {
        ...
        return querystring.stringify(param, '&', '=', { encodeURIComponent: o => o });
    }
};
const validateChain = (cert, x509Store) => {
    return new Promise(resolve => {
        pem.verifySigningChain(cert, x509Store, function (err, result) {
            resolve(err ? false : result);
        });
    });
};

const verifyAndGetVerifyKey = async (certBase64String) => {
    if (!STORE.x509Store) {
        STORE.x509Store = [fs.readFileSync(path.resolve(CERTS_PATH, 'acp_test_middle.cer')), fs.readFileSync(path.resolve(CERTS_PATH, 'acp_test_root.cer'))];
    }
    const c = new jsrsasign.X509();
    c.readCertPEM(certBase64String);
    const time = moment().format('YYMMDDHHmmss') + 'Z';
    const sNotBefore = c.getNotBefore();
    const sNotAfter = c.getNotAfter();
    if (time < sNotBefore || time > sNotAfter) {
        return null;
    }
    const sSubject = c.getSubjectString();
    const ss = sSubject.split('@');
    if (ss.length <= 2) {
        return null;
    }
    const cn = ss[2];
    if (cn != '中国银联股份有限公司' && cn != '00040000:SIGN') { // 测试环境目前是00040000:SIGN
        return null;
    }

    if (await validateChain(certBase64String, STORE.x509Store)) {
        return jsrsasign.X509.getPublicKeyFromCertPEM(certBase64String);
    } else {
        return null;
    }
};
export const validateByCert = async (data) => {
    const signature = data.signature;
    const key = await verifyAndGetVerifyKey(data.signPubKeyCert);
    if (!key) {
        return false;
    }
    const sig = new jsrsasign.Signature({ alg: 'SHA256withRSA' });
    sig.init(key);
    data = _.omit(data, 'signature');
    sig.updateString(jsrsasign.KJUR.crypto.Util.sha256(createLinkString(data, true, false)));
    return sig.verify(jsrsasign.b64tohex(signature));
};
export const signByCert = (data) => {
    const sig = new jsrsasign.Signature({ alg: 'SHA256withRSA' });
    sig.init(STORE.cert.privateKey);
    sig.updateString(jsrsasign.KJUR.crypto.Util.sha256(createLinkString(data, true, false)));
    return jsrsasign.hex2b64(sig.sign());
};
const request = async (url, params) => {
    if (!STORE.cert) {
        await initSignCert(path.resolve(CERTS_PATH, 'acp_test_sign.pfx'), '000000');
    }
    const data = {
        ...CONFIG,
        ...params,
        certId: STORE.cert.certId,
        txnTime: moment().format('YYYYMMDDHHmmss'), // 商户发送交易时间
        payTimeout: moment().add(7, 'd').format('YYYYMMDDHHmmss'), //超过此时间用户支付成功的交易，不通知商户，系统自动退款，大约7个工作日金额返还到用户账户
    };
    data.signature = signByCert(data);
...
export default {
    async unifiedorder (params) {
        return await request(SERVER + 'appTransReq.do', params);
    },
};

```

* PDShopServer/project/App/routers/posts/libs/unionPay/notifyCallBack.js

```js
import { UnionPayModel } from '../../../../models';
import { _T, omitNil } from '../../../../utils/';
import moment from 'moment';
import CONSTANTS from '../../../../constants';
import { rechargeForClient, rechargeForShopMember } from '../account';
import { validateByCert } from './api';
import config from '../../../../../config';

export default async (data) => {
    if (!await validateByCert(data)) {
    ...
    if (data.respCode === '00') {
        ...
        const order = await UnionPayModel.findOne({
            _id: orderId,
        ...
        if (order) {
            await order.update(omitNil({
                ...
                payedTime: moment(moment().format('YYYY') + traceTime, 'YYYYMMDDHHmmss').valueOf(),
                queryId,
                traceNo,
                traceTime,
                settleDate,
            ...
            const result = await recharge({
                userId: _T(userId),
                amount: txnAmt * (config.appPayRate || 1), // 单位：分
                // 第三方的货单号  0：支付宝(A)， 1：财付通(C)， 2：银联(Y)，3：工商银行(G)，4：建设银行(J)， 5：中国银行（Z）， 6：农业银行(N)，规则：缩写字母#第三方的货单号#第三方账号
                thirdpartyAccount: `Y#${queryId}#${traceNo}`,
            });
            if (result.success) {
...
const CONFIG = {
    ...
    notify_url: 'http://120.78.65.221/api/weixinPayNotifyCallBack',
};
...
export default async ({ xml }, { io }) => {
    ...
    if (return_code === 'SUCCESS' && result_code === 'SUCCESS') {
        ...
        if (order) {
            ...
            const result = await recharge({
                userId: _T(userId),
                amount: total_fee * (config.appPayRate || 1), // 单位：分
            ...
            if (result.success) {
                io.emitTo(userId, 'RP_REFRESH_REMAIN_AMOUNT_NF'); // 发送刷新余额的通知
                await order.update({ state: CONSTANTS.PS_SUCCESS });
            }
        } else {
            return '<xml><return_code><![CDATA[FAIL]]></return_code></xml>';
...
async function getPageData (shopId, shipperId, type, params) {
    ...
    } else if (type === 'toconfirm') {
        criteria = { 'stateList.state': CONSTANTS.OS_READY_PHONE_CONFIRM };
    } else if (type === 'waitpick') {
        criteria = { 'stateList.state': CONSTANTS.OS_WAIT_CLIENT_PICK };
    } else if (type === 'toload') {
        criteria = { 'stateList.state': CONSTANTS.OS_READY_LOAD };
    } else if (type === 'onway') {
        criteria = { 'stateList.state': { $in: [ CONSTANTS.OS_ON_THE_WAY, CONSTANTS.OS_READY_RECEIVE ] } };
    } else if (type === 'success') {
        criteria = { 'stateList.state': CONSTANTS.OS_RECEIVE_SUCCESS };
    }
    if (fromPC) {
...
// ['待入库', '待打电话确认', '待自提', '待装车', '运输中', '成功']
const TYPES = ['tostore', 'toconfirm', 'waitpick', 'toload', 'onway', 'success'];
export default async ({ userId, shopId, type, ...params }) => {
    const client = await ClientModel.findById(userId);
    ...
    if (_.includes(TYPES, type)) {
        ...
        return {
            ...
            context: {
                ...
                toconfirm: await getPageData(shopId, client.shipperId, 'toconfirm', params),
                waitpick: await getPageData(shopId, client.shipperId, 'waitpick', params),
                toload: await getPageData(shopId, client.shipperId, 'toload', params),
...
export default async ({
    ...
    item = _.find(stateList, (o) => o.state === CONSTANTS.OS_READY_RECEIVE);
    ...
    if (item) {
        ...
        stateList.unshift({ state: CONSTANTS.OS_READY_RECEIVE, count });
    }
    order.stateList = _.filter(stateList, o => o.count > 0);
    ...
    // 它之前的货单都应该处于待收货状态
    if ((_.find(order.stateList, (o) => o.state === CONSTANTS.OS_READY_RECEIVE) || {}).count === order.totalNumbers) {
        const orderIdList = await OrderModel.getOrderIdListFromNode(order);
        orderIdList.length && await OrderModel.update({ _id: { $in: orderIdList } }, { 'stateList.0.state': CONSTANTS.OS_READY_RECEIVE, modifyTime: Date.now() }, { multi: true });
    }
...
export default async ({
    ...
    if (!isTransport) {
        ...
        // 需要为初始单添加上additialFee的信息，让收货人查看到
        const originOrder = await OrderModel.getOriginOrderFromNode(order.id);
        originOrder && await originOrder.update({ additionalFee: transportFee });
    }
    await order.save();
...
export default async ({
    ...
    if (order.state !== CONSTANTS.OS_ON_THE_WAY) {
        return { msg: '货单状态错误' };
    }
...
async function getPageData (shopId, shipperId, type, params) {
    ...
    if (type === 'tostore') {
        ...
        criteria = { 'stateList.state': { $in: [ CONSTANTS.OS_ON_THE_WAY, CONSTANTS.OS_ON_TRANSFER ] } };
    } else if (type === 'success') {
        criteria = { 'stateList.state': CONSTANTS.OS_RECEIVE_SUCCESS };
    }
    if (fromPC) {
...
export default async ({
    ...
    const result = await RoadmapModel.update({
        _id: { $in: roadmapIdList },
        $or: [{ isCityDistribute: false, sendDoorEnable: true }, { isCityDistribute: true }],
    }, { enable }, { multi: true });
    if (result.n) {
...
export default async ({
    ...
    value, // 如果mode为0和1时，正数为增加，负数为减少
    valueList, // 如果mode为0和1时，正数为增加，负数为减少
}) => {
    if (fromPC) {
        if (!value || !typeList || !typeList.length) {
            return { msg: '参数错误' };
        }
        valueList = _.fill(Array(typeList.length), value);
    }
    if (!valueList || !valueList.length || !typeList || !typeList.length) {
        return { msg: '参数错误' };
    }
    if (!await hasAuthority(userId, CONSTANTS.AH_MODIFY_ROADMAP, true)) {
        return { msg: '你没有修改路线的权限' };
    ...
    if (mode === 0) {
        _.forEach(lastCodeList, o => {
            ...
            item && _.forEach(typeList, k => { item[keys[k]] = valueList[k]; });
        });
        await roadmap.save();
    } else if (mode === 1) {
        _.forEach(lastCodeList, o => {
        ...
        _.forEach(lastCodeList, o => {
            const item = _.find(sendDoorList, m => m.sendDoorEndPointLastCode === o);
            item && _.forEach(typeList, k => { item[keys[k]] *= (valueList[k] / 100 + 1); });
        });
        await roadmap.save();
    }
...
async function getPageData (shipperId, shopId, type, keyword, fromPC, pageNo, pageSize) {
    ...
    if (type === 'onway') {
        criteria = { ...criteria, state: { $in: [ CONSTANTS.TS_ON_THE_WAY, CONSTANTS.TS_READY_EXIT_WAREHOUSE ] } };
    } else if (type === 'complete') {
...
async function getPageData (shopId, type, params) {
    ...
    if (type === 'toprintbarcode') {
        ...
        criteria = { 'stateList.state': { $in: [ CONSTANTS.OS_ON_THE_WAY, CONSTANTS.OS_ON_TRANSFER, CONSTANTS.OS_READY_PHONE_CONFIRM, CONSTANTS.OS_WAIT_CLIENT_PICK, CONSTANTS.OS_READY_RECEIVE ] } };
    } else if (type === 'success') {
        criteria = { 'stateList.state': CONSTANTS.OS_RECEIVE_SUCCESS };
    }
    if (fromPC) {
...
```js
import { ShopPartmentModel } from '../../../../models';
import { getKeywordCriteriaForPartment, omitNil, getOwnShop } from '../../../../utils';

export default async ({ userId, keyword, type, fromPC, pageNo, pageSize }) => {
    const shopId = await getOwnShop(userId);
    const criteria = getKeywordCriteriaForPartment(keyword, omitNil({ shopId, type }));
...
```js
import { ShopMemberModel, TruckModel, DriverModel } from '../../../../models';
import { hasAuthority, actionLog } from '../../../../utils';
import CONSTANTS from '../../../../constants';
...
* PDShopServer/project/App/routers/posts/shop/securityCheckPartment/letTruckPass.js

```js
import { ShopMemberModel, TruckModel, OrderModel } from '../../../../models';
import { hasAuthority, actionLog } from '../../../../utils';
import CONSTANTS from '../../../../constants';
...
export default async ({
    ...
    actionLog(userId, 'ShopMember', driverId, 'Driver', 'letTruckPass');
...
* PDShopServer/project/App/routers/posts/shop/receivePartment/getLastOrder.js

```js
import { OrderModel } from '../../../../models';

export default async ({ userId, orderId }) => {
    let order = await OrderModel.getLatestOrderFromOriginOrder(orderId);
    if (!order) {
        return { msg: '没有该货单' };
    }
    order = await OrderModel.findById(order.id)
...
export default async ({
    ...
    const order = await OrderModel.findByIdAndUpdate(preOrderId, {
        ...
        stateList: [ { state: CONSTANTS.OS_READY_PRINT_BAR_CODE, count: totalNumbers } ],
        placeOrderTime: Date.now(),
        $unset: { groupId: 1, agentId: 1, startPointLastCode: 1, startPoint: 1 },
    });
    await order.save();
...
```js
import { ShopMemberModel, OrderModel, OrderGroupModel, TruckModel, MediaModel } from '../../../../models';
import { hasAuthority, actionLog } from '../../../../utils';
...
export default async ({
    ...
    if (!(preOrder.state === CONSTANTS.OS_ON_THE_WAY || preOrder.state === CONSTANTS.OS_READY_SEND)) {
        return { msg: '该货单不能被中转' };
    }
    ...
    } else if (preOrder.isCityDistribute) {
        isCityDistribute = true;
        if (!preOrder.agent) {
            return { msg: '同城配送中的货单不能被中转' };
        }
    }
    const hasReadyReceive = !!_.find(preOrder.stateList, o => o.state === CONSTANTS.OS_READY_RECEIVE);
    let isClientPick, endPoint, endPointLastCode, sendDoorEndPoint, sendDoorEndPointLastCode;
    if (preOrder.isCityDistribute) { // 说明是收货点的单
        isClientPick = false;
        endPoint = preOrder.endPoint;
        endPointLastCode = preOrder.endPointLastCode;
        sendDoorEndPoint = undefined;
        sendDoorEndPointLastCode = 0;
    } else {
        isClientPick = isCityDistribute && !preOrder.isSendDoor;
        endPoint = isCityDistribute && !isClientPick ? preOrder.sendDoorEndPoint : preOrder.endPoint;
        endPointLastCode = isCityDistribute && !isClientPick ? preOrder.sendDoorEndPointLastCode : preOrder.endPointLastCode;
    ...
    await preOrder.update({
        ...
        'stateList.0.state': CONSTANTS.OS_ON_TRANSFER,
        $unset: { groupId: 1 },
    });
    // 处理货单群组
    if (preOrder.groupId) {
        const group = await OrderGroupModel.findByIdAndUpdate(preOrder.groupId, { $inc: {
            totalNumbers: -preOrder.totalNumbers,
            totalWeight: -preOrder.weight,
            totalSize: -preOrder.size,
            orderCount: -1,
        }, $pull: { orderIdList: preOrder.id } }, { new: true });
        if (group && !group.orderIdList.length) {
            await group.remove();
        }
    }
...
export default async ({
    ...
    if (order.state !== CONSTANTS.OS_READY_PRINT_BAR_CODE) {
        return { msg: '该货单状态不正确' };
    }
...
export default async ({
    ...
    if (state === CONSTANTS.OS_READY_DEDUCT_AND_PRINT_ORDER) {
        ...
        if (err === ERROR_HAVE_PAYED) {
            ...
            order.modifyTime = Date.now();
            await order.save();
            return { msg: '物流公司余额不足, 请选择其他公司的路线' };
        } else if (err === ERROR_SHIPPER_BOND_AMOUNT_NOT_ENOUGH) {
            order.deductError = true;
            order.modifyTime = Date.now();
            await order.save();
            return { msg: '物流公司保证金不足，请选择其他公司的路线' };
        } else if (err) {
            order.deductError = true;
            order.modifyTime = Date.now();
            await order.save();
    ...
    for (const orderId of orderIdList) {
        ...
        if (state === CONSTANTS.OS_READY_DEDUCT_AND_PRINT_ORDER) {
            ...
            if (err === ERROR_HAVE_PAYED) {
                ...
                order.modifyTime = Date.now();
                await order.save();
                failedList.push({ msg: '物流公司余额不足, 请选择其他公司的路线' });
                continue;
            } else if (err === ERROR_SHIPPER_BOND_AMOUNT_NOT_ENOUGH) {
                order.deductError = true;
                order.modifyTime = Date.now();
...
export default async ({
    ...
    for (const orderId of orderIdList) {
        ...
        if (order.state === CONSTANTS.OS_READY_PAYMENT) {
            const err = await payment(
                order,
                { id: _T(member.partmentId), transportFee: _F(order.needPayTransportFee), insuanceFee: _F(order.needPayInsuanceFee), transportProfit: _F(order.masterProfit + order.branchProfit) },
                { id: _T(order.shipper.chairManId), proxyCharge: _F(order.proxyCharge), deductAmount: _F(order.totalDesignatedFee - order.fee) },
            ...
            if (err === ERROR_HAVE_PAYED) {
                ...
                order.modifyTime = Date.now();
                await order.save();
                failedList.push({ msg: '物流公司保证金不足, 部分货单未完成支付' });
            } else if (err) {
                failedList.push({ msg: '系统出错, 部分货单未完成支付' });
                continue;
            }
        } else if (order.state === CONSTANTS.OS_READY_DEDUCT_AND_PRINT_ORDER) {
            const err = await deductAmount(order, _T(order.shipper.chairManId), order.shipper.id, _F(order.proxyCharge), _F(order.totalDesignatedFee - order.fee));
            if (err === ERROR_HAVE_PAYED) {
                order.stateList = [ { state: CONSTANTS.OS_READY_STOCK, count: order.totalNumbers } ];
                order.modifyTime = Date.now();
...
export default async ({
    ...
    const order = await OrderModel.findById(orderId);
    if (order.isTransferOrder) { // 如果是中转单， 需要修改它之前的货单状态
        if (order.isSenderRepresentShipper) { // 物流公司中转的修改为运输中状态
            await OrderModel.findByIdAndUpdate(order.preOrderId, { 'stateList.0.state': CONSTANTS.OS_ON_THE_WAY });
        } else { // 收货点中转的修改为待发货 (收货点专用)状态
            await OrderModel.findByIdAndUpdate(order.preOrderId, { 'stateList.0.state': CONSTANTS.OS_READY_SEND });
        }
    }
    await order.remove();
    if (order && order.photo) {
...
export default async ({
    ...
    if (order.state !== CONSTANTS.OS_READY_SELECT_ROADMAP) {
        return { msg: '该货单状态不正确' };
    }
    const roadmap = await RoadmapModel.findById(roadmapId)
    .populate({
...
* PDShopServer/project/App/routers/posts/shop/unionPay/getUnifiedOrder.js

```js
import { ShopMemberModel, UnionPayModel } from '../../../../models';
import { _F, _T, hasAuthority, actionLog } from '../../../../utils/';
import CONSTANTS from '../../../../constants';
import api from '../../libs/unionPay/api';
import config from '../../../../../config';
...
* PDShopServer/project/App/routers/posts/shop/unionPay/getUnifiedOrderForPC.js
...
* PDShopServer/project/App/routers/posts/shop/warehousePartment/scanOrder.js
...
export default async ({
    ...
    const warehouse = await WarehouseModel.findOne({ shopId: member.shopId, houseNo: { $in: warehouseNoList }, warehousePartmentId: member.partmentId });
    if (!warehouse) {
...
export default async ({
    ...
    return new Promise(async resolve => {
        ...
        actionLog(userId, 'ShopMember', amount, 'Number', 'weixinPayGetUnifiedOrder');
...
export default async ({
    ...
    return new Promise(async resolve => {
        ...
        actionLog(userId, 'ShopMember', amount, 'Number', 'weixinPayGetUnifiedOrderForPC');
