...
module.exports = {
    ...
    ROUTE_SCAN_ORDER_BY_WARE_HOUSE_PARTMENT: SERVER + 'scanOrderByWareHousePartment', // 库管部扫描获取订单详情
    ...
    ROUTE_CHECK_TRUCK_PASS: SERVER + 'checkTruckPass', // 保安部扫描获取司机信息
    ROUTE_LET_TRUCK_PASS: SERVER + 'letTruckPass', // 保安部放行
    ...
    ROUTE_GET_LAST_ORDER : SERVER + 'getLastOrder', //获取中转单数据
                ...
                <View style={styles.title}>
                    <Image source={app.img.chairman_date} style={styles.dateImg} />
                    {
                        type && <View style={styles.title}>
                            ...
                            <TouchableOpacity onPress={() => { this.showDatePicker(Picker, moment('2017-01-01'), moment('2050-12-31'), moment(), 'end'); }}>
...
const { DImage, Button } = COMPONENTS;
...
module.exports = React.createClass({
    ...
    doConfirm () {
        ...
        POST(app.route.ROUTE_LET_TRUCK_PASS, param, this.letTruckPassSuccess, true);
    },
    letTruckPassSuccess (data) {
        if (data.success) {
            Toast('入库成功');
            app.pop();
        } else {
        ...
        return (
            <View style={styles.container}>
                ...
                <Button
                    ...
                    确认{context && context.isEnter ? '入库' : '出库'}
                </Button>
            </View>
        );
    },
...
module.exports = React.createClass({
    ...
    render () {
        ...
        console.log('2121212',app.personal.info);
        return (
            <View style={styles.container}>
...
module.exports = React.createClass({
    statics: {
        title: '搬运队信息',
        rightButton: { title: '确定', handler: () => { app.scene.doConfirm&&app.scene.doConfirm(); } },
        leftButton: { handler: () => { app.scene.doBack&&app.scene.doBack(); } },
    },
    doBack () {
        this.props.refresh();
        app.pop();
    ...
    doConfirm () {
        ...
        if (!app.utils.check2PointNum(price)) {
            Toast('搬运价格必须为纯数字');
            return;
        }
        const param = {
                ...
                <View style={styles.TeamInfo}>
                    ...
                    <TouchableOpacity onPress={this.toModifySlogan}>
                        <View style={styles.row}>
                            ...
                            <Text style={styles.slogan} numberOfLines={1} ellipsizeMode='tail'>
                                {slogan}
                            </Text>
...
module.exports = React.createClass({
    statics: {
        title: '搬运队',
        rightButton: { title: '搬运行情', handler: () => { app.scene.showtransportQuotation&&app.scene.showtransportQuotation(); } },
    },
    getInitialState () {
        return {
...
module.exports = React.createClass({
    ...
    getOrderDetailByReceivePartment (orderId) {
        ...
        POST(app.route.ROUTE_GET_LAST_ORDER, param, this.getOrderDetailSuccess, false);
    },
    ...
    setInfo(data){
        this.setState({
            ...
            totalNumbers:data.context.totalNumbers,
            size:data.context.size,
            weight:data.context.weight,
         });
    },
    placeTransferOrder () {
            ...
            <View style={styles.container}>
                <View style={styles.topContainer}>
                    ...
                    <View style={styles.topRight}>
                        <Text numberOfLines={1} style={styles.topRightText}>始发地址：{context.shop && context.shop.address || context.agent && context.agent.name}</Text>
...
module.exports = React.createClass({
    statics: {
        title: '拍照',
        rightButton: { title: '扫描', handler: () => { app.scene.scanBarCode&&app.scene.scanBarCode(); } },
    },
    onWillFocus () {
        app.getCurrentRoute().title = '拍照';
        app.getCurrentRoute().rightButton = { title: '扫描', handler: () => { app.scene.scanBarCode&&app.scene.scanBarCode(); } };
        app.getCurrentRoute().leftButton = null;
        app.forceUpdateNavbar();
...
module.exports = React.createClass({
    ...
    getOrderDetail () {
        ...
        POST(app.route.ROUTE_SCAN_ORDER_BY_WARE_HOUSE_PARTMENT, param, this.scanOrderByWareHousePartmentSuccess, true);
    },
    scanOrderByWareHousePartmentSuccess (data) {
        const { success, retry, msg, context } = data;
        if (success || retry) {
            if (_.find(data.context.stateList, o => o.state == 5 && o.count)) {
                ...
                app.scanStoageOrder.saveOrder(data.context);
            } else {
                Toast('该货单已全部收货完成');
                app.pop();
            }
    ...
    render () {
        const { data, count, unStoageNumbers, msg, retry } = this.state;
        console.log('data',data,retry);
        return (
            <View style={styles.container}>
                <View style={styles.containerDetails}>
                    <DImage style={styles.imageGoods}
                ...
                <View style={styles.bottom}>
                    <Button style={styles.btnConfirm} textStyle={styles.btnFont} onPress={this.showScanResult}>{retry?'强制入库':'确认'}</Button>
...
module.exports = React.createClass({
    ...
    getPlaceStorageSuccess (data) {
        if (data.success) {
            ...
            if (data.retry) {
                this.setState({force:!!data.retry,msg:data.msg,list:data.context,unStoageNumbers: _.find(data.context.stateList, o => o.state == 5 && o.count).count});
            } else {
